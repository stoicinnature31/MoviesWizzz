...
const MovieInfo = ({movie}) => {
  return (
    <div className='w-full xl:h-screen relative text-white'>
      <img src={`${movie?.i.imageUrl}`} alt={movie.name} className='w-full hidden xl:inline-block h-full object-cover' />

      <div className='xl:bg-main bg:dry flex-colo xl:bg-opacity-90 xl:absolute top-0 left-0 right-0 bottom-0'>
        <div className='container px-3 mx-auto 2xl:px-32 xl:grid grid-cols-3 flex-colo py-10 lg:py-20 gap-8'>
          <div className='xl:col-span-1 w-full xl:order-none order-last h-header bg-dry'>
            <img src={`${movie?.i.imageUrl}`} alt={movie?.name} className='w-full h-full object-cover' />
          </div>

          <div className='col-span-2 md:grid grid-cols-5 gap-4 items-center'>
            <div className='col-span-3 flex flex-col gap-10'>
              {/* Title */}
              <h1 className='xl:text-4xl capitalize font-sans text-2xl font-bold text-sky-500'>{movie?.name}</h1>
              ...
              <div className='grid sm:grid-cols-4 grid-cols-3 gap-4 p-6 bg-main '>
                ...
                <div className='col-span-2 flex-colo font-medium text-sm'>
                  <p>Category : {' '} <span className='ml-2 truncate'>{movie?.category}</span></p>
                </div>

                {/* Watch Button */}
                <div className='sm:col-span-2 col-span-3 flex justify-end font-medium text-sm'>
                  <Link to={`/watch/${movie?.name}`} className='bg-dry hover:bg-sky-500 transitions border-2 border-dry flex-rows gap-4 w-full sm:py-5 py-5'>
